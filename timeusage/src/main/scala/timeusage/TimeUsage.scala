...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, DataTypes.StringType, nullable = false) ::
      columnNames.tail.map(s => StructField(s, DataTypes.DoubleType, nullable = false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val groups: Map[Int, List[Column]] =
      columnNames
        .foldLeft(List.empty[(Int, Column)])((acc, name) =>
          categoryMappings
            .flatMap {
              case (prefixes, i) if prefixes.exists(name.startsWith) => Some((i, new Column(name)))
              case _ => None
            }
            .sortBy(_._1)
            .headOption match {
    ...
    val List(l1, l2, l3) = categoryMappings.indices.map(groups.getOrElse(_, List.empty[Column])).toList
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(d: Double) = BigDecimal(d).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble
